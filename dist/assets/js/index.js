...
function formatPhoneSpan(phoneStr) {
    if (!phoneStr) return '';
...
async function loadSiteData() {
    ...
    try {
        ...
        if (footerPhone && data.contact?.phone) {
            ...
            footerPhone.href = `tel:+34${cleanPhone}`;
            footerPhone.textContent = formatPhoneSpan(data.contact.phone);
        }
        ...
        if (contactPhone && data.contact?.phone) {
            const cleanPhone = data.contact.phone.replace(/\D/g, '');
            contactPhone.href = `tel:+34${cleanPhone}`;
            contactPhone.textContent = formatPhoneSpan(data.contact.phone);
        }
